...
const TCE_GLOBAL_PATTERN = "(?:^|[;,])\\s*(var\\s+([\\w$]+)\\s*=\\s*\"(?:[^\"\\\\]|\\\\.)+\"\\s*\\.\\s*split\\(\"([^\"\\\\]|\\\\.)\"\\))(?=\\s*[,;])"
...
const extractDecipherFunc = body => {
  try {
    ...
    return extractDecipherFuncTce(body);
  }
};

const extractDecipherFuncTce = body => {
  try {
    const helperObject = matchFirst(HELPER_REGEXP, body);
    const decipherFunc = matchFirst(DECIPHER_REGEXP_TCE, body);
    const findFunc = decipherFunc.match(/function.*?;/)[0];
    const resultFunc = decipherFunc.replace(findFunc, "");
    const callerFunc = `${DECIPHER_FUNC_NAME}(${DECIPHER_ARGUMENT});`;
    return `${findFunc.replace("function", `function ${DECIPHER_FUNC_NAME}`)}` + helperObject + resultFunc + callerFunc;
...
const extractNTransformFunc = body => {
  try {
    ...
    const resolveNFunc = nFunc.replace(/if\s*\(\s*typeof\s*[\w$]+\s*===?.*?\)\s*return\s.*;/, '');
    const resultFunc = `var ${N_TRANSFORM_FUNC_NAME}=${resolveNFunc}`;
    const callerFunc = `${N_TRANSFORM_FUNC_NAME}(${N_ARGUMENT});`;
    return resultFunc + callerFunc;
  } catch {
    return extractNTransformFuncTce(body);
  }
};

const extractNTransformFuncTce = body => {
  try {
    const nFunc = matchFirst(N_TRANSFORM_REGEXP_TCE, body);
    var tceVars = matchFirst(TCE_GLOBAL_PATTERN, body);
    
    if (tceVars.startsWith(";")) {
      tceVars = tceVars.slice(1);
    }
    ...
    const checkEmptySplit = tceVars.match(/\.split\(["'](.*?)["']\)/);

    let isMobileTce = false;
    let mobileTceVars = "";

    if (checkEmptySplit) {
      if (checkEmptySplit[1] === "" || checkEmptySplit[1] === " ") {
        mobileTceVars = matchFirst(/var\s+(\w+)=\s*(?:"([^"]*)"|'([^']*)'\.split\(";"\)|\[((?:".*?"|'.*?')(?:\s*,\s*(?:".*?"|'.*?'))*)\])/g, body);
        isMobileTce = true;
      }
    }

    if (mobileTceVars.startsWith(";")) {
      mobileTceVars = mobileTceVars.slice(1);
    }

    if (!mobileTceVars.endsWith(";")) {
      mobileTceVars += ";";
    }

    const resolveNFunc = nFunc.replace(/if\s*\(typeof\s*[^\s()]+\s*===?.*?\)\s*return\s.*;/, '');
    const resultFunc = `var ${N_TRANSFORM_FUNC_NAME}=${resolveNFunc}`;
    const callerFunc = `${isMobileTce ? mobileTceVars : tceVars}${N_TRANSFORM_FUNC_NAME}(${N_ARGUMENT});`;
